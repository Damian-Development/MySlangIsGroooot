var QLearning = (function () {
  ...
  function saveToCookies() {
    // Save qTable, score, and missed to cookies
    setCookie('qTable', JSON.stringify(qTable), 7); // Save for 7 days
    setCookie('score', score, 7);
    setCookie('missed', missed, 7);
  }

  function loadFromCookies() {
    // Load qTable, score, and missed from cookies
    var savedQTable = getCookie('qTable');
    if (savedQTable) qTable = JSON.parse(savedQTable);

    var savedScore = getCookie('score');
    if (savedScore) score = parseInt(savedScore, 10);

    var savedMissed = getCookie('missed');
    if (savedMissed) missed = parseInt(savedMissed, 10);
  }
  ...
  return {
    ...
    reset: function () {
      ...
      score = 0;
      missed = 0;
      deleteCookies(); // Delete cookies when resetting
    },
